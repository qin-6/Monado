...
/*!
 * A custom type
 * fix foveation render parameters, that sits on  @ref xrt_device
 * 
 */
struct xrt_ffr_bundle
{
	float center_size_x;
	float center_size_y;
	...
	char vert_shader[XRT_DEVICE_NAME_LEN];
	char frag_shader[XRT_DEVICE_NAME_LEN];

	bool (*ffr_enable)(void *xdev);
};
/*!
...
struct xrt_device
{
	 ...
	 *@brief custom 1
	 */
	bool (*streaming_initialize)(struct xrt_device *xdev, void *vk_device);
	/*!
     *@brief custom 2
	 */
	bool (*streaming_onframe)(struct xrt_device *xdev, void *frame, uint64_t desired_timestamp_ns);
	/*!
	 *@brief custom 3
	 */
	bool (*streaming_check_resolution)(struct xrt_device *xdev, uint32_t *width, uint32_t *height);
	/*!
	 *@brief custom 4
	 */
	bool (*streaming_update_resolution)(struct xrt_device *xdev, uint32_t width, uint32_t height);
};
