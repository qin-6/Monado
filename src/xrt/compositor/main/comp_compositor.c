...
compositor_begin_session(struct xrt_compositor *xc, enum xrt_view_type type)
{
	...
	if(xdev->streaming_initialize)
	{
		struct vk_device vk_dev;
		vk_dev.vkInstance 				= c->base.vk.instance;
		vk_dev.vkDevice   				= c->base.vk.device;
		vk_dev.vkPhysicDevice			= c->base.vk.physical_device;
		vk_dev.vkInstanceGetProcAddrFn 	= c->base.vk.vkGetInstanceProcAddr;
		vk_dev.vkGraphicQueue			= c->base.vk.queue;
		vk_dev.queueFamilyIndex 		= c->base.vk.queue_family_index;
		xdev->streaming_initialize(xdev, &vk_dev);
	}
                         ...
                         uint64_t *out_predicted_display_period_ns)
{
	...
	//COMP_INFO(c, "assert id = %d", c->frame.waited.id);

	assert(comp_frame_is_invalid_locked(&c->frame.waited));

	//COMP_INFO(c, "calc frame pacing before");
	int64_t frame_id = -1;
	...
	//COMP_INFO(c, "frame id = %d, wakeup time = %d, desired time = %d, preset slop = %d, predict time = %d",
	//frame_id, wake_up_time_ns, desired_present_time_ns, present_slop_ns, predicted_display_time_ns);
...
do_graphics_layers(struct comp_compositor *c)
{
	...
	for (uint32_t i = 0; i < layer_count; i++) {
		...
		case XRT_LAYER_STEREO_PROJECTION: {								//双目
...
select_target_factory_from_settings(struct comp_compositor *c, const struct comp_target_factory **out_ctf)
{
	...
	COMP_INFO(c, "select target from settings, identifier = %s", identifier);
	if (identifier == NULL) {
...
select_target_factory_by_detecting(struct comp_compositor *c, const struct comp_target_factory **out_ctf)
{
	for (size_t i = 0; i < ARRAY_SIZE(ctfs); i++) {
		...
		{
			COMP_INFO(c, "select target by detecting, target name = %s", ctf->name);
			*out_ctf = ctf;
...
compositor_init_window_pre_vulkan(struct comp_compositor *c, const struct comp_target_factory *selected_ctf)
{
	...
	COMP_INFO(c, "comp target not find yet!! ctfs size = %d", ARRAY_SIZE(ctfs));
	for (size_t i = 0; i < ARRAY_SIZE(ctfs); i++) {
		const struct comp_target_factory *ctf = ctfs[i];
