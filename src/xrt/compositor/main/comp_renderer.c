...
renderer_ensure_images_and_renderings(struct comp_renderer *r, bool force_recreate)
{
	...
	//COMP_INFO(c, "comp target check ready tag 3");
	...
	//COMP_INFO(c, "recreate needed : %d", create);
	if (!create) {
		return true;
	}
	...
	//COMP_INFO(c, "comp target create images before");
	...
	if (!render_distortion_images_ensure(&r->c->nr, &r->c->base.vk, r->c->xdev, pre_rotate))
		return false;
	//COMP_INFO(c, "comp target create images after count : %d", r->c->target->image_count);
...
dispatch_graphics(struct comp_renderer *r, struct render_gfx *rr)
{
	...
	//COMP_INFO(c, "layer fast path = %d, frame rendering id = %lld", one_projection_layer_fast_path, c->frame.rendering.id);
	...
	//COMP_INFO(c, "layer data type = %d", layer->data.type);
...
comp_renderer_draw(struct comp_renderer *r)
{
	...
	//COMP_INFO(c, "dispatch graphics success single eye width = %d, height = %d", r->lr->extent.width, r->lr->extent.height);
	//COMP_INFO(c, "both width = %d, height = %d", c->target->width, c->target->height);
#ifdef XRT_FEATURE_WINDOW_PEEK
	if (c->peek) {
...
#endif
	//COMP_INFO(c, "present,  rendering id = %d", r->c->frame.rendering.id);
	...
	if (comp_target_check_ready(r->c->target)) {
		...
		//COMP_INFO(r->c, "comp target check ready tag 2");
	}
	//COMP_INFO(r->c, "update timings after preset");
	comp_target_update_timings(ct);
}
