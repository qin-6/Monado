...
do_update_timings_google_display_timing(struct comp_target_swapchain *cts)
{
	...
	COMP_INFO(cts->base.c, "count = %d", count);
	...
	for (uint32_t i = 0; i < count; i++) {
		u_pc_info(cts->upc,                       //
		          ...
		          now_ns); 						  //
		COMP_INFO(cts->base.c, "resentID = %d, desiPresentTime = %d, actPtime = %d, earliestPTime = %d, pM = %d",
						timings[i].presentID, timings[i].desiredPresentTime, timings[i].actualPresentTime, 
						timings[i].earliestPresentTime, timings[i].presentMargin);	          
	}
                                    ...
                                    VkPresentModeKHR present_mode)
{
	...
	COMP_INFO(ct->c, "---------->input format  =%d", color_format);
	...
	COMP_INFO(ct->c, "prefered_width = %d, prefered_height = %d", preferred_width, preferred_height);
	...
	COMP_INFO(ct->c, "extent width = %d, extent height = %d", extent.width, extent.height);
	COMP_INFO(ct->c, "--------supported format = %d", cts->surface.format.format);
...
{
	struct comp_target_swapchain *cts = (struct comp_target_swapchain *)ct;
	COMP_INFO(ct->c, "frame_id = %lld, current_frame_id = %lld", frame_id, cts->current_frame_id);
	assert(frame_id == cts->current_frame_id);
	COMP_INFO(ct->c, "point = %d", point);
	switch (point) {
